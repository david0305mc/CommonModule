...
{
    public class DavidTiles3D_Anchor : MonoBehaviour
    {
        [SerializeField]
        ...
        public List<DavidTiles3D_BlockBehaviour> GetBlocks()
        {
            var children = GetComponentsInChildren<DavidTiles3D_BlockBehaviour>(true).ToList();
            _childCount = children.Count();
            return children;
        }

        public DavidTiles3D_TileLayer Layer => transform.GetComponentInParent<DavidTiles3D_TileLayer>();
        ...
        {
            var tile = DavidTiles3D_Utility.GetTile(_tileID, "", "");
            if (tile != null)
            {
                var blocks = GetBlocks();
                foreach (var block in blocks)
                {
                    ...
                    {
                        DavidTiles3D_Utility.RepairBlocks(block, blocks, tile.Group, tile.Name, tile.TileID);
                    }
                }
            }
        }
