...
{
    [CustomEditor(typeof(DavidTiles3D_TileLayer))]
    public class DavidTiles3D_TileLayerInspector : Editor
    {
        DavidTiles3D_TileLayer _tileLayer;
        DavidTiles3D_Grid Grid => _tileLayer.Grid;
        ...
        List<DavidTiles3D_BlockBehaviour> _brokenBlocks = new List<DavidTiles3D_BlockBehaviour>();
        ...
        {
            _tileLayer = (DavidTiles3D_TileLayer)target;
            _tileLayer.Grid = _tileLayer.GetComponentInParent<DavidTiles3D_Grid>();
            DavidTiles3D_TileLayer.IS_EDITING = true;
            ...
            _tileLayer.LoadedGroups = DavidTiles3D_Utility.LoadTileGroups();
        ...
        private void OnSceneGUI()
        {
            ...
            if (DavidTiles3D_TileLayer.SHOW_LAYER_OUTLINE)
            {
            ...
            if (!DavidTiles3D_TileLayer.IS_EDITING)
                return;
            ...
            if (e.shift && (!_isPulling && !_isPushing))
            {
                ...
                if (!_tileLayer.IsLayerEmpty)
                {
                    if (DavidTiles3D_GridRaycast.GridRayCast(_tileLayer, _MouseRay.origin, _MouseRay.GetPoint(50), out Vector3Int internalHit, out Vector3Int internalHitNormal, out visits))
                    {
                        ...
                        {
                            DavidTiles3D_BlockBehaviour block = _tileLayer.GetInternalNode(internalHit).Block;
                            ...
                            if (_PullMode == PullMode.Face)
                            {
                                if (_tileLayer.ContainsKey(internalHit))
                                {
                                    DavidTiles3D_Tile tile = _tileLayer.GetInternalNode(internalHit).GetTile();
                                    if (tile != null)
                                    {
                            ...
                            else if (_PullMode == PullMode.Plane)
                            {
                                ...
                                var nodes = DavidTiles3D_EditorUtility.GetAllUnblockedContAdjacentNodesDepthFirst(_tileLayer, internalHit, _faceNormalInternal).ToList();
                                foreach (var node in nodes)
                                {
                                    if (_tileLayer.ContainsKey(node))
                                    {
                                        DavidTiles3D_Tile tile = _tileLayer.GetInternalNode(node).GetTile();
                                        if (tile != null)
                                        {
                        ...
                        {
                            Debug.LogError("DavidTiles3D: InternalNode GridRaycast went wrong.");
                        }
                    }
                }
                ...
                if (e.type == EventType.MouseDown)
                {
                    if (e.button == 0 || e.button == 1 && _selectedNodes.Count > 0)
                    {
                        ...
                        if (succesfullHit)
                        {
                            ...
                            _pIndex = 0;
                            DavidTiles3D_Settings.IsLocked = true;
                            e.Use();
                        }
                    }
            ...
            else
            {
                ...
                DavidTiles3D_Settings.IsLocked = false;
            }
            ...
            if (DavidTiles3D_TileLayer.SHOW_HOVER_GIZMO)
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (DavidTiles3D_TileLayer.SHOW_HOVER_GIZMO)
            {
                ...
                {
                    DavidTiles3D_TileLayer.SHOW_HOVER_GIZMO = false;
                }
            }
            else
            {
                ...
                {
                    DavidTiles3D_TileLayer.SHOW_HOVER_GIZMO = true;
                }
            }
            if (DavidTiles3D_TileLayer.SHOW_LAYER_OUTLINE)
            {
                ...
                {
                    DavidTiles3D_TileLayer.SHOW_LAYER_OUTLINE = false;
                }
            }
            else
            {
                ...
                {
                    DavidTiles3D_TileLayer.SHOW_LAYER_OUTLINE = true;
                }
            }
            if (!DavidTiles3D_TileLayer.IS_EDITING)
            {
            ...
            {
                EditorWindow.GetWindow<DavidTiles3D_SettingsWindow>("DavidTiles3D Settings", typeof(SceneView));
            }
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                */
                ...
                foreach (var tile in _tileLayer.Tiles)
                {
                        ...
                        else
                        {
                            if (EditorUtility.DisplayDialog($"Bake all {tile.Name} meshes?", $"Do you want to bake all {tile.Name} meshes into a single one? \nAll baked meshes will be disabled, but not deleted. You can always re-enable them later or even rebake your meshes after more changes to the layer have been made.", "Yes", "No"))
                            {
                                ...
                                if (anchor != null && anchor.Childcount > 0)
                                {
                                    ...
                                    string path = "Assets/DavidTiles3D/Content/CombinedMeshes";
                                    var blocks = anchor.GetBlocks();
                                    DavidTiles3D_MeshCombiner.CombineMeshes(blocks.Select(c => c.gameObject).ToList(), ref anchor.BakedParent, path);
                    ...
                    if (amount >= 500 && !DavidTiles3D_Settings.EditorInstance.SuppressTileAmountWarning)
                    {
            ...
            {
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                EditorGUILayout.LabelField($"<color=red>{_totalBrokenAmt} Broken tiles found! </color>", DavidTiles3D_Utility.RichStyle);
                ...
                EditorGUILayout.LabelField("Don't worry, this is <color=cyan>easy to fix </color>!", DavidTiles3D_Utility.RichStyle);
                ...
                for (int i = 0; i < _broken.Count; i++)
                {
                    ...
                    if (GUILayout.Button("Link"))
                    {
                        if (group != null)
                        {
                            ...
                            if (tile != null)
                            {
                                DavidTiles3D_BlockBehaviour block = _brokenBlocks[i];
                                if (block != null)
                                    DavidTiles3D_Utility.RepairBlocks(block, block.Anchor.GetBlocks(), group.name, tile.Name);
                            }
                        }
        ...
        {
            DavidTiles3D_TileLayer.IS_EDITING = false;
            _tileLayer.DestroyHoverInstance();
            OnInspectorGUI();
        }
        ...
        public void RenderHoverInstance(Vector3Int internalPosition, Quaternion localRotation)
        {
            ...
            var activeTile = DavidTiles3D_Utility.GetTile(_tileLayer.ActiveTileID);
