...
namespace DavidTiles3D
{
    ...
    {
        public DavidTiles3D_TileLayer Layer;
        ...
        [SerializeField] DavidTiles3D_BlockBehaviour _block;
        ...
        public InternalNode(DavidTiles3D_TileLayer layer, string group, string tileName, int tileID, Vector3Int internalPosition, Quaternion localRotation, GameObject instance = null)
        {
            Layer = layer;
...
#if UNITY_EDITOR
        public DavidTiles3D_Tile GetTile()
        {
            return DavidTiles3D_Utility.GetTile(_tileID, _tileName, _group);
        }
#endif
        ...
        public DavidTiles3D_BlockBehaviour Block
        {
            get
            {
                if (_block == null)
                {
                    if (_instance != null)
                    {
                        _block = _instance.GetComponent<DavidTiles3D_BlockBehaviour>();
                        if (_block == null)
                        {
                            //if instance is missing, add blockbehaviour
                            _block = _instance.AddComponent<DavidTiles3D_BlockBehaviour>();
    ...
    public class DavidTiles3D_TileLayer : MonoBehaviour, ISerializationCallbackReceiver
    {
        public string LayerName;
        private Dictionary<Vector3Int, InternalNode> _internalNodes = new Dictionary<Vector3Int, InternalNode>();
        public Dictionary<int, DavidTiles3D_Anchor> Anchors = new Dictionary<int, DavidTiles3D_Anchor>(); // tileID, anchor
        ...
        public DavidTiles3D_Anchor GetAnchor(int tileID)
        {
            if (Anchors.ContainsKey(tileID))
                return Anchors[tileID];
            return null;
        }
        public DavidTiles3D_TileGroup Group;
        public List<DavidTiles3D_Tile> Tiles => Group != null ? Group.Tiles : new List<DavidTiles3D_Tile>();
        ...
        private DavidTiles3D_Grid _Grid;
        public DavidTiles3D_Grid Grid
        {
            get
            {
                if (_Grid == null)
                    _Grid = GetComponentsInParent<DavidTiles3D_Grid>(includeInactive: true)[0];
                return _Grid;
            }
            set
            {
        ...
        [SerializeField] private List<DavidTiles3D_Anchor> _AnchorValues = new List<DavidTiles3D_Anchor>();
        ...
        public void OnAfterDeserialize()
        {
            ...
            Anchors = new Dictionary<int, DavidTiles3D_Anchor>();
        ...
        public List<DavidTiles3D_TileGroup> LoadedGroups = new List<DavidTiles3D_TileGroup>();
        ...
        {
            DavidTiles3D_Anchor[] anchors = GetComponentsInChildren<DavidTiles3D_Anchor>();
            ...
            foreach (var anchor in anchors)
            {
                ...
                {
                    Debug.LogWarning($"DavidTiles3D: Duplicate Anchor {anchor.TileID} {anchor.name}");
                }
            }
        }
        ...
        {
            var tile = DavidTiles3D_Utility.GetTile(tileID);
            if (tile == null)
                return;
        ...
        public void UpdateAllImmediate(DavidTiles3D_Anchor anchor)
        {
            if (anchor == null)
                return;
        ...
        public void VerifyAllImmediate(DavidTiles3D_Anchor anchor)
        {
            if (anchor == null)
                return;

            List<DavidTiles3D_BlockBehaviour> blocks = anchor.GetBlocks();
            foreach (var block in blocks)
            {
                if (_internalNodes.ContainsKey(block.InternalPosition))
                {
        ...
        private void AddNodeInternal(int tileId, string tileName, string group, Vector3Int internalPosition, Quaternion localRotation)
        {
            if (!_internalNodes.ContainsKey(internalPosition))
            {
                ...
                if (!Anchors.TryGetValue(tileId, out DavidTiles3D_Anchor anchor))
                {
        ...
        private void RemoveNodeInternal(Vector3Int internalPosition)
        {
            if (_internalNodes.TryGetValue(internalPosition, out InternalNode node))
            {
                ...
                if (node.Block != null)
                {
                    if (node.Block.IsBaked)
                    {
                        Debug.Log("DavidTiles3D: Will not remove baked blocks.");
                        return;
                    }
                }
        ...
        public void ToggleView(DavidTiles3D_Tile tile, bool enable)
        {
            var matches = _internalNodes.Where(p => p.Value.TileID == tile.TileID).ToList();
            foreach (var match in matches)
                match.Value.Block?.ToggleView(enable);
        }
        public void RemoveAllBlocks(DavidTiles3D_Anchor anchor)
        {
            if (anchor != null)
            {
                var blocks = anchor.GetBlocks();
