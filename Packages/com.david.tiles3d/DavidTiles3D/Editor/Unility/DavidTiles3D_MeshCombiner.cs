...
{
	public static class DavidTiles3D_MeshCombiner
	{
		...
		public static bool CombineMeshes(List<GameObject> roots, ref GameObject newParent, string path)
		{
			...
			{
				Debug.LogError("DavidTiles3D/MeshCombiner: Invalid root objects");
				return false;
			}
			...
			foreach (var meshFilter in meshFilters)
			{
				...
				{
					Debug.LogWarning("DavidTiles3D/MeshCombiner: The Mesh Filter on object " + meshFilter.name + " has no Mesh Renderer component attached. Skipping.");
					continue;
				}
				...
				if (materials == null)
				{
					Debug.LogWarning("DavidTiles3D/MeshCombiner: The Mesh Renderer on object " + meshFilter.name + " has no material assigned. Skipping.");
					continue;
				}
				...
				if (materials.Length > 1)
				{
					...
					Debug.LogError("DavidTiles3D/MeshCombiner: Objects with multiple materials on the same mesh are not supported. Create multiple meshes from this object's sub-meshes in an external 3D tool and assign separate materials to each. Operation cancelled.");
					return false;
				}
